using System.Collections.Generic;
using Elsa.Activities.Conductor;
using Elsa.ActivityResults;
using Elsa.Attributes;
using Elsa.Design;
using Elsa.Expressions;
using Elsa.Services.Models;

namespace AspNetCoreDemoApp.Activities.Events
{

    [Action(
        Category = "Events",
        DisplayName = "OnLogin",
        Description = "On Login Event",
        Outcomes = new[] { LoggedInMachineSchema.State }
    )]
    public class LoginEvent : EventReceived
    {
        [ActivityInput(Hint = "The name of this state.",
            DefaultValue = "OnLogin",
            SupportedSyntaxes = new[] { SyntaxNames.JavaScript, SyntaxNames.Liquid })]
        public string StateName { get; set; } = default!;

        [ActivityInput(
            Hint = "Enter one or more transition names.",
            UIHint = ActivityInputUIHints.MultiText,
            DefaultSyntax = SyntaxNames.Json,
            DefaultValue = new[] { LoggedInMachineSchema.State },
            SupportedSyntaxes = new[] { SyntaxNames.Json },
            IsDesignerCritical = true
        )]
        public ISet<string> Transitions { get; set; } = new HashSet<string>();


        protected override IActivityExecutionResult OnExecute(ActivityExecutionContext context)
        {
            context.SetVariable("CurrentState", StateName);
            context.JournalData.Add("Current State", StateName);
            return Outcomes(Transitions);
        }
    }
}