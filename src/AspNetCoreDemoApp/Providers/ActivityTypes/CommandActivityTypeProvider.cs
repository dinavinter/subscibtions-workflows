...
namespace AspNetCoreDemoApp.Providers.ActivityTypes
{
    public class ActionActivityTypeProvider : IActivityTypeProvider
    {
        ...
        public ActionActivityTypeProvider(IDescribesActivityType describesActivityType, IActivityActivator activityActivator, Scoped<IEnumerable<ICommandsProvider>> scopedCommandsProviders)
        {
        ...
        private async Task<ActivityType> CreateActivityTypeAsync(CommandDefinition command, CancellationToken cancellationToken)
        {
            ...
            {
                var des = await _describesActivityType.DescribeAsync<ExecuteAction>(cancellationToken);

                 des.Type = command.Name;
                des.DisplayName = command.DisplayName ?? command.Name;
                des.Description = command.Description;
                des.InputProperties = des.InputProperties.Where(x => x.Name != nameof(ExecuteAction.CommandName)).ToArray();
            ...
            return new ActivityType
            {
                ...
                {
                    var activity = await _activityActivator.ActivateActivityAsync<ExecuteAction>(context, cancellationToken);
