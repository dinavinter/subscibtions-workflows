...
{
    public class ServiceActivityTypeProvider : IActivityTypeProvider
    {
        ...
        public ServiceActivityTypeProvider(IDescribesActivityType describesActivityType, IActivityActivator activityActivator, Scoped<IEnumerable<ITasksProvider>> scopedTasksProviders)
        {
        ...
        private async Task<ActivityType> CreateActivityTypeAsync(TaskDefinition taskDefinition, CancellationToken cancellationToken)
        {
            ...
            {
                var des = await _describesActivityType.DescribeAsync<RunService>(cancellationToken);
            ...
            return new ActivityType
            {
                ...
                {
                    var activity = await _activityActivator.ActivateActivityAsync<RunService>(context, cancellationToken);
